#!/usr/bin/env python3
"""
Debug HVACR Scraper - Ultra Methodical
...
OUTPUT_DIR = Path(__file__).parent.parent / "output" / "state_licenses" / "new_jersey" / "debug_hvacr"
...
async def debug_hvacr():
    """Debug HVACR scrape step by step."""
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Check if HVACR is in the list
            hvacr_option = [opt for opt in options if 'HVACR' in opt['text']]
            if hvacr_option:
                print(f"   ✅ 'HVACR' found in dropdown:")
                print(f"      value: '{hvacr_option[0]['value']}'")
                print(f"      text: '{hvacr_option[0]['text']}'")
            else:
                print(f"   ❌ 'HVACR' NOT found in dropdown!")
