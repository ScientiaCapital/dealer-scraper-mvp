#!/usr/bin/env python3
"""
Apply Year 1 GTM-aligned ICP scoring to EXPANDED 10-OEM grandmaster list

ENHANCED with HVAC capability detection for MEP+R scoring

Scoring Dimensions:
  - Resimercial (35%): Residential + Commercial capability
  - Multi-OEM (25%): Managing multiple OEM platforms
  - MEP+R (25%): Multi-trade (Mechanical, Electrical, Plumbing + Roofing) **ENHANCED**
...
def score_resimercial(contractor: Dict) -> Tuple[int, str]:
    ...
    # HVAC contractors often serve both markets (NEW!)
    has_hvac = contractor.get('has_hvac') == True or contractor.get('has_hvac') == 'True'
    ...
    elif is_elite and (has_generators or has_hvac):
        score = 75
        evidence.append("Elite generator/HVAC dealer (likely serves both markets)")
    elif has_generators or has_hvac:
        score = 50
        evidence.append("Generator/HVAC dealer (mixed market)")
    else:
        score = 40
...
def score_mepr(contractor: Dict) -> Tuple[int, str]:
    ...
    **ENHANCED**: Checks has_hvac capability flag for automatic Mechanical trade
    Returns: (score, evidence)
    """
    name = normalize_text(contractor.get('name', ''))
    ...
    # **NEW**: Check has_hvac capability flag (automatic Mechanical trade!)
    has_hvac = contractor.get('has_hvac') == True or contractor.get('has_hvac') == 'True'
    if has_hvac:
        trades['Mechanical'] = True

    # Detect mechanical (HVAC) from name
    ...
    # Build evidence
    evidence_parts = []
    if has_hvac and 'Mechanical' in active_trades:
        evidence_parts.append("HVAC contractor (Mechanical)")
...
def score_om(contractor: Dict) -> Tuple[int, str]:
    ...
    # HVAC contractors often have service contracts (NEW!)
    has_hvac = contractor.get('has_hvac') == True or contractor.get('has_hvac') == 'True'
    ...
    elif has_hvac and 'service' in name:
        score = 70
        evidence = "HVAC service contractor (maintenance contracts)"
...
def main():
    print("=" * 80)
    print("APPLYING YEAR 1 GTM ICP SCORING - EXPANDED 10-OEM GRANDMASTER")
    print("=" * 80)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("\n📊 Scoring Dimensions:")
    print("   • Resimercial (35%): Residential + Commercial")
    print("   • Multi-OEM (25%): Multiple OEM platforms")
    print("   • MEP+R (25%): Multi-trade capability **ENHANCED WITH HVAC FLAG**")
    print("   • O&M (15%): Maintenance service contracts")
    print("=" * 80)

    # Load EXPANDED grandmaster list
    timestamp = datetime.now().strftime("%Y%m%d")
    input_file = f'output/grandmaster_list_expanded_{timestamp}.csv'

    print(f"\n📂 Loading expanded grandmaster list: {input_file}")
    ...
    # Count capability flags
    hvac_count = sum(1 for c in contractors if c.get('has_hvac') == 'True' or c.get('has_hvac') == True)
    solar_count = sum(1 for c in contractors if c.get('has_solar') == 'True' or c.get('has_solar') == True)

    print(f"\n📊 Capability Flags Detected:")
    print(f"   HVAC contractors: {hvac_count} (will boost MEP+R scoring)")
    ...
    # Save scored results
    output_file = f"output/icp_scored_contractors_final_{timestamp}.csv"
    ...
    priority_fields = [
        ...
        'has_hvac', 'has_solar', 'has_inverters',
    ...
    top_200_file = f"output/top_200_prospects_final_{timestamp}.csv"
    ...
    hvac_in_200 = sum(1 for c in top_200 if c.get('has_hvac') == 'True' or c.get('has_hvac') == True)

    print(f"\n   Multi-OEM contractors in Top 200: {multi_oem_200}")
    print(f"   HVAC contractors in Top 200: {hvac_in_200}")
