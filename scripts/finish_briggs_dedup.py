#!/usr/bin/env python3
"""
Load Briggs checkpoint and finish deduplication
"""
import json
import csv
...
# Load checkpoint
print("Loading Briggs checkpoint...")
with open("output/briggs_checkpoint_140_20251027_191230.json", 'r') as f:
...
tiers = {}
for dealer in deduped_dealers:
    tier = dealer.tier or 'Standard'
    tiers[tier] = tiers.get(tier, 0) + 1

print(f"\n   Dealers by tier:")
for tier in sorted(tiers.keys(), key=lambda t: tiers[t], reverse=True):
    print(f"      {tier}: {tiers[tier]}")

print(f"\n✅ Briggs deduplication complete!")
