#!/usr/bin/env python3
"""
Production run: Generac Dealer Locator - ALL 50 STATES
...
def save_checkpoint(dealers, checkpoint_num, timestamp):
    ...
    json_file = f"output/generac_national_checkpoint_{checkpoint_num}_{timestamp}.json"
    dealers_dict = [d.to_dict() for d in dealers]
    with open(json_file, 'w') as f:
        json.dump(dealers_dict, f, indent=2)

    # Save CSV checkpoint
    csv_file = f"output/generac_national_checkpoint_{checkpoint_num}_{timestamp}.csv"
...
def main():
    print("=" * 70)
    print("GENERAC BACKUP GENERATORS - NATIONAL RUN (ALL 50 STATES)")
    print("=" * 70)
    ...
    csv_file = f"output/generac_national_{timestamp}.csv"
    json_file = f"output/generac_national_{timestamp}.json"

    scraper.save_to_csv(csv_file)
    scraper.save_to_json(json_file)

    print(f"\n✅ Generac national run complete!")
