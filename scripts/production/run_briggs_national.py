#!/usr/bin/env python3
"""
Production run: Briggs & Stratton Dealer Locator - ALL 50 STATES
...
def save_checkpoint(dealers, checkpoint_num, timestamp):
    ...
    json_file = f"output/briggs_national_checkpoint_{checkpoint_num}_{timestamp}.json"
    dealers_dict = [d.to_dict() for d in dealers]
    with open(json_file, 'w') as f:
        json.dump(dealers_dict, f, indent=2)

    # Save CSV checkpoint
    csv_file = f"output/briggs_national_checkpoint_{checkpoint_num}_{timestamp}.csv"
...
def main():
    print("=" * 70)
    print("BRIGGS & STRATTON STANDBY GENERATORS - NATIONAL RUN (ALL 50 STATES)")
    print("=" * 70)
