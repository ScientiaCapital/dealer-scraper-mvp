#!/usr/bin/env python3
"""
Production Briggs & Stratton dealer scraping for all 140 SREC state ZIP codes
...
def save_progress(all_dealers, checkpoint_num, timestamp):
    ...
    json_file = f"output/briggs_checkpoint_{checkpoint_num}_{timestamp}.json"
    dealers_dict = [d.to_dict() for d in all_dealers]
    with open(json_file, 'w') as f:
        json.dump(dealers_dict, f, indent=2)

    # Save CSV checkpoint
    csv_file = f"output/briggs_checkpoint_{checkpoint_num}_{timestamp}.csv"
...
def main():
    print("="*70)
    print("BRIGGS & STRATTON PRODUCTION SCRAPING")
    print("="*70)
    ...
    for i, zip_code in enumerate(ZIP_CODES_SREC_ALL, 1):
        print(f"\n[{i}/{len(ZIP_CODES_SREC_ALL)}] Scraping Briggs & Stratton dealers for ZIP {zip_code}...")
    ...
    deduped_dealers = scraper.deduplicate(all_dealers)
    ...
    # Save JSON
    json_file = f"output/briggs_dealers_{final_timestamp}.json"
    ...
    # Save CSV
    csv_file = f"output/briggs_dealers_{final_timestamp}.csv"
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        if deduped_dealers:
            fieldnames = [
                'name', 'phone', 'domain', 'website',
    ...
    print(f"\n✅ Briggs & Stratton production scraping complete!")
